...
const statusBadge = (status) => {
  const map = {
    pending:    { label: "অপেক্ষারত",   bg: "#FEF3C7", color: "#92400E", dot: "#F59E0B" },
    confirmed:  { label: "নিশ্চিত",      bg: "#D1FAE5", color: "#065F46", dot: "#10B981" },
    processing: { label: "প্রক্রিয়াধীন", bg: "#DBEAFE", color: "#1E40AF", dot: "#3B82F6" },
    shipped:    { label: "পাঠানো হয়েছে", bg: "#EDE9FE", color: "#4C1D95", dot: "#8B5CF6" },
    delivered:  { label: "ডেলিভারি সম্পন্ন", bg: "#D1FAE5", color: "#065F46", dot: "#10B981" },
    cancelled:  { label: "বাতিল",        bg: "#FEE2E2", color: "#7F1D1D", dot: "#EF4444" },
  };
...
const orderConfirmationCustomer = (order) => {
  const {
    orderNumber, customerName, customerEmail, shippingAddress = {},
    items = [], subtotal, shippingCost, totalAmount,
    paymentMethod, status = "confirmed", createdAt,
  } = order;
  ...
  return wrap(`
    ...
    <tr>
      <td class="email-pad" style="padding:0 48px 36px;">
        ...
        <table width="100%" cellpadding="0" cellspacing="0" border="0">
          ${[
            { step:"১", label:"অর্ডার নিশ্চিত", desc:"আমরা আপনার অর্ডার পেয়েছি", done:true },
            { step:"২", label:"প্রক্রিয়াকরণ", desc:"পণ্য প্যাকেজিং চলছে", done:false },
            { step:"৩", label:"শিপমেন্ট", desc:"কুরিয়ারে পাঠানো হবে", done:false },
            { step:"৪", label:"ডেলিভারি", desc:"আপনার দরজায় পৌঁছাবে", done:false },
          ].map(s => `
            <tr>
...
const orderStatusUpdateCustomer = (order) => {
  ...
  const statusMessages = {
    processing: { headline:"আপনার অর্ডার প্রস্তুত হচ্ছে!", body:"আমাদের টিম আপনার পণ্য যত্নসহকারে প্যাক করছে।", emoji:"📦" },
    shipped:    { headline:"অর্ডার পাঠানো হয়েছে!", body:"আপনার পণ্য রাস্তায় আছে। শীঘ্রই আপনার কাছে পৌঁছাবে।", emoji:"🚚" },
    delivered:  { headline:"ডেলিভারি সম্পন্ন!", body:"আপনার পণ্য পৌঁছে গেছে। কেনাকাটায় ধন্যবাদ!", emoji:"🎉" },
    cancelled:  { headline:"অর্ডার বাতিল করা হয়েছে", body:"আপনার অর্ডারটি বাতিল হয়েছে। কোনো প্রশ্ন থাকলে যোগাযোগ করুন।", emoji:"❌" },
  };

  const msg = statusMessages[status] || { headline:"অর্ডার আপডেট", body:"আপনার অর্ডারে পরিবর্তন হয়েছে।", emoji:"ℹ️" };
...
const newOrderAdmin = (order) => {
  const {
    ...
    items = [], subtotal, shippingCost, totalAmount,
    paymentMethod, status, shippingAddress = {}, createdAt, notes,
  } = order;
        ...
        ">
          ...
          <table width="100%" cellpadding="0" cellspacing="0" border="0">
            ${[
              ...
              ["ইমেইল", customerEmail],
              ["ফোন", customerPhone],
              ["ঠিকানা", [shippingAddress.address, shippingAddress.city, shippingAddress.zipCode].filter(Boolean).join(", ")],
            ].map(([label, value]) => `
              <tr>
...
// ═══════════════════════════════════════════════════════════════════
const lowStockAdmin = (products = []) => wrap(`
  ${header("স্টক সতর্কতা")}
