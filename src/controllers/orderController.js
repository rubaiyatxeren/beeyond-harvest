...
// ─── Track in-flight email promises so graceful shutdown can await them ───────
const pendingEmails = new Set();

const trackEmail = (promise) => {
  pendingEmails.add(promise);
  promise.finally(() => pendingEmails.delete(promise));
};

// Call this in your server.js SIGTERM handler:
// await drainEmails();
const drainEmails = () => Promise.allSettled([...pendingEmails]);
module.exports.drainEmails = drainEmails; // exported so server.js can use it
...
// ─── Internal: send all emails for an order, tracked for drain ───────────────
const sendOrderEmails = (order, type = "new_order") => {
  const job = (async () => {
    try {
      const customerHtml = generateOrderEmailTemplate(order, type);
      const customerResult = await sendEmail(
        order.customer.email,
        type === "new_order"
          ? `🎉 Order Confirmed - ${order.orderNumber}`
          : `📦 Order ${order.orderNumber} Status Updated to ${order.orderStatus.toUpperCase()}`,
        customerHtml,
      );
      console.log(
      ...
      // Admin emails only on new orders
      if (type === "new_order") {
        const adminEmails = process.env.ADMIN_EMAILS
          ? process.env.ADMIN_EMAILS.split(",")
          : ["ygstudiobd@gmail.com"];
        const adminHtml = generateAdminEmailTemplate(order, type);
        for (const email of adminEmails) {
          ...
          console.log(
            result?.success
              ...
              : `⚠️ Admin email failed (${email.trim()}): ${result?.error}`,
          );
        }
      }
    } catch (err) {
...
const createOrder = async (req, res) => {
  try {
    ...
    // ✅ Respond immediately — emails fire after
    res.status(201).json({
      success: true,
      data: order,
    ...
    // 🔔 Background emails — tracked so SIGTERM waits for them
    if (process.env.DISABLE_EMAIL !== "true") {
      sendOrderEmails(order, "new_order");
    }
  } catch (error) {
...
const updateOrderStatus = async (req, res) => {
  try {
    ...
    // 🔔 Background email — tracked
    if (oldStatus !== newStatus && process.env.DISABLE_EMAIL !== "true") {
      sendOrderEmails(order, "status_update");
    }
  } catch (error) {
